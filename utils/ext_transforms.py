...
class ExtToTensor(object):
    ...
    def __call__(self, pic, lbl):
        ...
        if isinstance(pic, np.ndarray):
            pic = torch.from_numpy(pic.transpose(2, 0, 1)).float() / 255.0
        else:  # Assume input is a PIL Image
            pic = F.to_tensor(pic)
        
        # Convert label to tensor without normalization
        if isinstance(lbl, np.ndarray):
            lbl = torch.from_numpy(lbl).type(torch.__dict__[self.target_type])
        else:  # Assume input is a PIL Image
...
class ExtResize(object):
    ...
    def __call__(self, img, lbl):
        ...
        img_resized = F.resize(img, self.size, self.interpolation)
        lbl_resized = F.resize(lbl, self.size, Image.NEAREST)
